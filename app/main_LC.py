...
def load_model_and_tokenizer(model_name):
    global model, tokenizer, llm_chain  # Declarar llm_chain para LangChain
    ...
    # Crear el pipeline de Hugging Face para LangChain
    hf_pipeline = HuggingFacePipeline(pipeline=pipeline("text-generation", model=model, tokenizer=tokenizer))
    
    # Configurar el prompt y cadena en LangChain
    prompt = ChatPromptTemplate.from_template("### Human: {prompt}\n\n### Relevant Info: {retrieved_info}\n\n### Assistant:")
    output_parser = StrOutputParser()
    llm_chain = LLMChain(llm=hf_pipeline, prompt=prompt, output_parser=output_parser)
...
async def generate_text(request_body: RequestBody):
    ...
    # Generar respuesta utilizando la cadena de LangChain
    response = llm_chain.run({"prompt": prompt, "retrieved_info": retrieved_info})
